...
public class MemoryFileEx {
     ...
     */
    public MemoryFileEx(String name, int length) throws IOException {
        mLength = length;
    ...
    public MemoryFileEx(int fd, int length) throws IOException, InvalidAlgorithmParameterException {
        FileDescriptor jfd = getFileDescriptor(fd);
    ...
    public int readBytes(byte[] buffer, int srcOffset, int destOffset, int count)
        ...
        if (com.rsupport.litecam.binder.Binder.IS_ROOTED)
            return native_read(mFD, mAddress, buffer, srcOffset, destOffset, count, mAllowPurging);
        else
            return native_read_rs(mFD, mAddress, buffer, srcOffset, destOffset, count, mAllowPurging);
    }
    ...
    public void writeBytes(byte[] buffer, int srcOffset, int destOffset, int count)
        ...
        if (com.rsupport.litecam.binder.Binder.IS_ROOTED)
            ...
            native_write_rs(mFD, mAddress, buffer, srcOffset, destOffset, count, mAllowPurging);
    }
    ...
    private static FileDescriptor native_open(String name, int length)
            throws IOException {
        Method m = get("native_mmap", new Class[]{FileDescriptor.class,
                int.class, int.class});
        try {
    ...
    private static int native_mmap(FileDescriptor fd, int length, int mode)
            throws IOException {
        Method m = get("native_mmap", new Class[]{FileDescriptor.class,
                int.class, int.class});
        try {
    ...
    private static void native_munmap(int addr, int length) throws IOException {
        Method m = get("native_munmap", new Class[]{int.class, int.class});
        try {
            m.invoke(null, addr, length);
        } catch (IllegalAccessException e) {
            e.printStackTrace();
        } catch (IllegalArgumentException e) {
            e.printStackTrace();
    ...
    private static int native_read_rs(FileDescriptor fd, int address,
    ...
    private static void native_write_rs(FileDescriptor fd, int address,
                                        byte[] buffer, int srcOffset, int destOffset, int count,
                                        boolean isUnpinned) throws IOException {
        try {
